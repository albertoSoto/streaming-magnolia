        ...
        <div class="video-wrapper">
            [#if content.type == "asset"]
                <video ${content.assetautoplay?string("autoplay","")} ${content.assetloop?string("loop","")} ${content.assetmuted?string("muted","")} ${content.assetcontrols?string("controls","")} ${preload!} ${poster!}>
                    <source src="/.spring/streaming?fileRQ=${videoSrc}" type="${assetType}">
                </video>
            [#--    <video ${content.assetautoplay?string("autoplay","")} ${content.assetloop?string("loop","")} ${content.assetmuted?string("muted","")} ${content.assetcontrols?string("controls","")} ${preload!} ${poster!}>
                    <source src="/.spring/manual?fileRQ=${videoSrc}" type="${assetType}">
                </video>--]
                [#--<video ${content.assetautoplay?string("autoplay","")} ${content.assetloop?string("loop","")} ${content.assetmuted?string("muted","")} ${content.assetcontrols?string("controls","")} ${preload!} ${poster!}>
                    <source src="/.spring/download2?fileRQ=${videoSrc}" type="${assetType}">
                </video>--]
            [#else]
